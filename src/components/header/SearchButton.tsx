...
function SearchPanel() {
  ...
  if (import.meta.env.DEV) {
    resultList = (
      <div className="h-full flex items-center justify-center">
        <div className="flex gap-2">
          ...
          <div>
            <div className="font-semibold mb-1">Désolé</div>
            <div className="text-sm">Cette fonctionnalité est basée sur Pagefind. Veuillez réessayer après la compilation.</div>
          </div>
        </div>
      </div>
    )
    ...
    resultList = (
      <>
        <div className="text-sm px-3 mb-2">J'ai {results.length} résultats</div>
        {results.map((item) => {
          return (
            <a
              href={item.url}
